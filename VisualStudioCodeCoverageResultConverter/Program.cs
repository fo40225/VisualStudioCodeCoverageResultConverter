...
namespace VisualStudioCodeCoverageResultConverter
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            {
                string input = args.Where(x => x.StartsWith("/input:", StringComparison.InvariantCultureIgnoreCase))
                    .Select(x => Regex.Replace(x, "/input:", string.Empty, RegexOptions.IgnoreCase)).First();
                string output = args.Where(x => x.StartsWith("/output:", StringComparison.InvariantCultureIgnoreCase))
